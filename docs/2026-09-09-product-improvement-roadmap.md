...
**Status update (18 September 2026) — Home simplification + savings review shipped:** Beranda now leads with **Kekayaan Bersih** and **Bisa dipakai sekarang** (renamed from "Dana yang bisa dipakai saat ini"). The ledger balance and the saved checkpoint balance appear as one merged row, **Uang kamu**, in Rincian saldo; duplicate Kekayaan Bersih/Dana rows were removed from that list. The hero shows a **Disisihkan di tabungan** pill (goal reservations + unassigned + needs-review) that reconciles Beranda's "Bisa dipakai sekarang" with Rencana's "Tersedia untuk dibagi", and links into the new savings review screen: `SavingsReviewModal` inside Rencana > Target lists Tabungan rows with blank goal metadata and assigns them to a goal or releases them through the existing `/api/savings/allocations` pipeline (fingerprints verified server-side, one operation id, no new endpoint). Release immediately raises "Bisa dipakai sekarang"; assignment moves the money into the goal's allocation. Naming rule going forward: one concept, one label, one place — hero numbers are never repeated inside Rincian saldo.
