...
- Tasks completed: Final review Product & IA migration fix pass; corrected `Beranda` urgent-action routing so budget actions deep-link to `Rencana > Budget` and bill actions deep-link to `Rencana > Tagihan`; added minimal shared `PlanTab` section state; removed `EventBudgetsSection` ownership from `Statistik`; wrote final fix report.
- Files changed: `src/app/dashboard/page.js`, `src/app/dashboard/HomeTab.jsx`, `src/app/dashboard/PlanTab.jsx`, `src/app/dashboard/StatsTab.jsx`, `tests/components/HomeTab.test.jsx`, `tests/components/PlanTab.test.jsx`, `tests/components/StatsTab.test.jsx`, `.superpowers/sdd/final-fix-report.md`, `.superpowers/sdd/progress.md`, `progress.md`
- Decisions: Kept the 4-tab shell intact; preserved `Fokus hari ini` on `Beranda`; used the smallest controlled/uncontrolled `PlanTab` state change needed to support deep-linking without breaking local tab switching.
- Blockers: Prompt-referenced `final-fix-brief.md` was found in repo root `.superpowers/sdd/` rather than inside the worktree; focused IA verification is green, while broader suite/build verification was intentionally out of scope for this final-pass task.
- Tasks completed: Updated What-If Scenario semantics so the expense input now represents the desired future monthly spend for the selected category; derived goal acceleration from `category average - target spend`; aligned helper text, summary text, and instructions with that behavior.
